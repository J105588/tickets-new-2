// system-lock.ts - システムロック機能
import errorHandler from './error-handler.js';
// Dynamic import for GasAPI to avoid circular dependency
// import { GasAPI } from './api.js';

declare global {
  interface Window {
    systemLockReady?: Promise<any>;
    systemLock: SystemLock;
  }
}

class SystemLock {
  private _resolveReady?: (value?: unknown) => void;
  private _readyResolved: boolean;
    ...
    try {
      ...
      const tick = async () => {
        try {
          ...
          const status = await (GasAPI as any).getSystemLock();
